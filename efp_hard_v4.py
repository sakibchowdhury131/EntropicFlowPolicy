"""
Entropic Flow Policy — Hard Experiment v4
==========================================
Builds on v3 (failure demonstration supervision). Two targeted fixes for
the v3 regression: EFP dropped from 72% → 56% because:

  (a) Failure demos approached obstacles from ALL angles, including from
      the right (goal side). This contaminated the goal-approach corridor
      with high entropy, blocking the policy from completing trajectories.

  (b) The entropy gradient near obstacles became very steep (5.3 wall vs
      3.87 success ceiling). With the raw gradient used in guidance, the
      policy was over-steered — bouncing off the wall rather than routing
      smoothly around it.

Fixes:
  1. Left-approach only: failure demos start at x < obstacle x_min and
     walk rightward into the obstacle. The goal-approach region (right of
     obstacles) stays clean.
  2. Boundary-only labeling: only the last `boundary_steps` states before
     collision are labeled H_FAILURE. Earlier approach states are dropped,
     making the entropy wall narrow and localised at the boundary.
  3. Normalised entropy gradient: gradient is scaled to unit length before
     applying guidance. This separates steering direction from gradient
     magnitude, preventing steep walls from over-correcting the policy.
"""
...
def main():
    print("="*60)
    print("ENTROPIC FLOW POLICY — Multi-Modal Obstacle Navigation v4")
    print("  + Failure Supervision (left-approach, boundary-only, normalised grad)")
    print("="*60)
    ...
    fig.suptitle('Entropic Flow Policy v4 — Failure Supervision (left-approach, boundary-only, normalised grad)',
    ...
    ax.plot(*env.goal, 'r*', ms=14, zorder=5)
    ax.set_xlim(-3.1,3.1); ax.set_ylim(-3.1,3.1); ax.set_aspect('equal')
    ax.set_title('Learned Entropy Landscape (v4)', fontsize=11)
    ...
    ax.set_title('Training Losses (v4)', fontsize=11)
    ...
    out_path = '/home/sakib/working_dir/entropyFlowPolicy/efp_v4_results.png'
