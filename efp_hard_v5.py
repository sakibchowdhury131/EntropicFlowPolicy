"""
Entropic Flow Policy — Hard Experiment v5
==========================================
All v4 fixes retained. One new change: closed gap.

The core research claim — that EFP handles multi-modal demonstrations where
BC mode-averaging fails — was never testable in v2/v3/v4 because the 0.8-unit
gap between the two obstacles allowed BC's mean action to sneak through the
middle, giving BC a trivial 100% success rate.

Fix: close the gap entirely. Obstacles now form a solid wall from y=-2.5 to
y=2.5 with only a 0.1-unit slot at centre (too narrow to navigate). Expert
demos are strictly upper (y > 2.5) or lower (y < -2.5) paths — no middle
mode. BC's mean action now averages upper and lower trajectories and points
directly into the solid wall → BC must fail on this task. EFP, guided by the
entropy landscape, should detect the two distinct basins and commit to one.

This is the first version where the task design actually forces the
multi-modality problem that EFP is designed to solve.
"""
...
class ObstacleNavEnv:
    """
    2D point navigation with two rectangular obstacles forming a near-solid wall.

    Layout:
        Workspace: [-3, 3] x [-3, 3]
        Obstacle 1: rectangle at x ∈ [-0.3, 0.3], y ∈ [0.05, 2.5]
        Obstacle 2: rectangle at x ∈ [-0.3, 0.3], y ∈ [-2.5, -0.05]
        Residual slot: y ∈ [-0.05, 0.05] — 0.1 units, impassable in practice

    This creates TWO viable paths:
        - UPPER: arc above obstacle 1 (y > 2.5)
        - LOWER: arc below obstacle 2 (y < -2.5)

    BC's mean action averages upper and lower demos → points at y≈0 → hits
    the solid wall. EFP must detect the bimodal entropy landscape and commit.
    ...
    def __init__(self):
        ...
        # Obstacles (x_min, x_max, y_min, y_max) — gap closed to 0.1 units
        self.obstacles = [
            (-0.3, 0.3, 0.05, 2.5),   # upper block (was 0.4)
            (-0.3, 0.3, -2.5, -0.05), # lower block (was -0.4)
        ]
...
def generate_demos(env, n_demos=300, n_points=50):
    ...
    for i in range(n_demos):
        ...
        # Only upper/lower — no middle mode (gap is closed)
    ...
    print(f"  Upper: {modes.count('upper')}, Lower: {modes.count('lower')}  (no middle — gap closed)")
    return demos


def generate_failure_demos(env, n_per_obstacle=60, boundary_steps=6):
    """
    Generate left-approach-only failure trajectories with boundary labeling.

    Key constraints vs v3:
    - Start strictly to the LEFT of each obstacle (x < xmin). This keeps the
      goal-approach corridor (right of obstacles) free of high-entropy signal.
    - Only the last `boundary_steps` states before collision are retained and
      labeled H_FAILURE. The earlier approach states are discarded. This creates
      a narrow entropy wall at the obstacle boundary rather than a broad corridor.
    """
    demos = []
    ...
    for xmin, xmax, ymin, ymax in env.obstacles:
        ...
        while generated < n_per_obstacle and attempts < n_per_obstacle * 15:
            ...
            # Start strictly left of the obstacle, within (or near) its y span
            start_x = np.random.uniform(-2.5, xmin - 0.1)
            start_y = np.random.uniform(ymin - 0.5, ymax + 0.5)
            start = np.array([start_x, start_y])
            if env.in_obstacle(start):
                continue

            # Target: a random point inside the obstacle
            target = np.array([
            ...
            for _ in range(50):
                ...
                if dist < 1e-3:
                    break
                action = (direction / dist) * env.max_speed
                next_pos = pos + action * env.dt
                next_pos = np.clip(next_pos, -3, 3)
                ...
                if env.in_obstacle(pos):
                    # Two steps inside so the boundary is clearly labeled
                    for _ in range(2):
                        all_steps.append({
                            'state': pos.copy(),
                            'action': np.zeros(2),
            ...
            if not hit or len(all_steps) < 2:
                continue

            # Keep only the boundary_steps states nearest the obstacle
            boundary = all_steps[-boundary_steps:]
            for step in boundary:
                step['entropy_label'] = H_FAILURE
            demos.append(boundary)
            generated += 1

    total_steps = sum(len(d) for d in demos)
    print(f"Generated {len(demos)} failure trajectories ({total_steps} boundary states)")
    return demos
    ...
    with torch.no_grad():
        for k in reversed(range(K)):
            ...
            if guidance > 0 and k > 0 and k % 2 == 0:
                ...
                # Normalise to unit length: separates steering direction from
                # gradient magnitude so steep failure walls don't over-correct.
...
def main():
    print("="*60)
    print("ENTROPIC FLOW POLICY — Multi-Modal Obstacle Navigation v5")
    print("  + Failure Supervision + Closed Gap (strictly upper/lower paths)")
    print("="*60)
    ...
    fig.suptitle('Entropic Flow Policy v5 — Closed Gap: Strictly Upper/Lower Paths',
    ...
    ax.plot(*env.goal, 'r*', ms=14, zorder=5)
    ax.set_xlim(-3.1,3.1); ax.set_ylim(-3.1,3.1); ax.set_aspect('equal')
    ax.set_title('Learned Entropy Landscape (v5)', fontsize=11)
    ...
    ax.set_title('Training Losses (v5)', fontsize=11)
    ...
    out_path = '/home/sakib/working_dir/entropyFlowPolicy/efp_v5_results.png'
